...
With MyCte as 
(
select cn.Channel_Name, nullif(cr.csat_score, '') as csat 
from contact_raw cr left join channel cn on cr.channel_id = cn.Channel_ID
)
select Channel_Name, round(avg(csat),2) as C_Sat
from MyCte
...
group by q.query_type; -- 2nd method
